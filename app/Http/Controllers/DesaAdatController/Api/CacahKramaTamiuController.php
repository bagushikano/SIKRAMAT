...
class CacahKramaTamiuController extends Controller
{
    ...
    public function get() {
        ...
        $kramas = CacahKramaTamiu::with('penduduk')->whereIn('banjar_adat_id', $arr_banjar_id)->paginate(10);
        if ($kramas != null) {
    ...
    public function detail($krama_id) {
        $krama = CacahKramaTamiu::with('penduduk', 'banjar')->where('id', $krama_id)->first();
        if ($krama != null) {
            return response()->json([
                'statusCode' => 200,
                'status' => true,
