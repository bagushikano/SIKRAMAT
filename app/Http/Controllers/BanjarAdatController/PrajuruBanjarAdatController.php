...
class PrajuruBanjarAdatController extends Controller
{
    ...
    public function datatable(Request $request){
        ...
        $prajuru_banjar_adat = PrajuruBanjarAdat::with('krama_mipil.cacah_krama_mipil.penduduk', 'krama_mipil.cacah_krama_mipil.tempekan', 'user')->where('banjar_adat_id', $banjar_adat_id);
        if(isset($request->status)){
            if($request->status == '1'){
                $prajuru_banjar_adat->where('status', '1');
            }else if($request->status == '0'){
                $prajuru_banjar_adat->where('status', '0');
            }
        }else{
            $prajuru_banjar_adat->where('status', '1');
        }
        ...
        if(isset($request->rentang_waktu)){
            ...
            $prajuru_banjar_adat->whereBetween('tanggal_mulai_menjabat', [$start_date, $end_date])->whereBetween('tanggal_akhir_menjabat', [$start_date, $end_date]);
        }

        return DataTables::of($prajuru_banjar_adat)
            ->addIndexColumn()
    ...
    {
        return view('pages.banjar.prajuru.prajuru');
    }
    ...
    public function store(Request $request)
    {
        ...
        //Create Prajuru
        $prajuru_desa_adat = new PrajuruBanjarAdat();
        $prajuru_desa_adat->user_id = $user->id;
        $prajuru_desa_adat->krama_mipil_id = $request->krama_mipil;
        $prajuru_desa_adat->banjar_adat_id = $banjar_adat_id;
        ...
        return back()->with('success', 'Prajuru Banjar Adat Berhasil Ditambahkan');
    }

    public function edit($id)
    {
        $prajuru = PrajuruBanjarAdat::with('krama_mipil.cacah_krama_mipil.penduduk')->find($id);
    ...
    {
        $prajuru = PrajuruBanjarAdat::find($id);
        $user = User::find($prajuru->user_id);
        ...
        return back()->with('success', 'Prajuru Banjar Adat berhasil diperbaharui');
    }
    ...
    public function destroy($id)
    {
        ...
        $user->role = 'krama';
        $user->update();
        return back()->with('success', 'Prajuru Banjar Adat berhasil dinonaktifkan');
    }
