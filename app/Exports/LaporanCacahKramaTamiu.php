...
class LaporanCacahKramaTamiu implements FromView, WithStyles, ShouldAutoSize, WithEvents, WithColumnWidths, WithColumnFormatting
{
    protected $kramaTamiu;

    public function __construct($kramaTamiu, $request) {
    ...
    public function view(): View
    {
        ...
        $data = [
            'krama_tamiu' => $this->kramaTamiu
        ];

        if ($this->request->banjar_adat_tamiu) {
            return view("vendor.export.excel.desa_adat.laporan-cacah-krama-tamiu", compact('data'));
        } else{
            return view("vendor.export.excel.banjar_adat.laporan-cacah-krama-tamiu", compact('data'));
        }
    }
