...
<head>
    ...
    <title>SIKRAMAT | Verifikasi Email</title>
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="text-center mb-3">
                                    <h4 class="text-black">Verifikasi Email</h4>
                                    {{-- <p class="text-muted">Sistem Informasi Manajemen Kependudukan Desa Adat Terintegrasi</p>  --}}
                                </div> 
                                <div class="p-3">
                                    @if (session('resent'))
                                        <div class="alert alert-success" role="alert">
                                            {{ __('Email verifikasi telah dikirimkan') }}
                                        </div>
                                    @endif
                                    <p class="text-center">
                                        {{ __('Silahkan melakukan verifikasi email sebelum dapat melanjutkan aksi') }}
                                    </p>
                                    <br>
                                    <p class="text-center font-weight-bold">
                                        {{ __('Jika email tidak diterima, tekan tombol dibawah') }}
                                    </p>

                                    <form method="POST" action="{{ route('verification.resend') }}">
                                        @csrf                                       
                                      <button type="submit" class="btn btn-primary btn-rounded btn-lg btn-block mt-2">Kirim Email</button>
                                    </form>
                                </div>
                            </div>
                        </div>
