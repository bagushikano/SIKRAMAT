            ...
            <div class="container">
                <div class="page-header-content pt-4">
                    <div class="row align-items-center justify-content-between">
                        <div class="col-auto mt-4">
                            <h1 class="page-header-title">
                                ...
                                Perkawinan Dalam Desa Desa Adat
                            </h1>
                    ...
                    <ol class="breadcrumb mb-0 mt-4">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('desa-krama-mipil-home') }}" class="text-decoration-none">Perkawinan</a></li>
                    ...
                    <div class="nav nav-pills nav-justified flex-column flex-xl-row nav-wizard" id="cardTab" role="tablist">
                        <a class="nav-item nav-link active bg-gray-200" id="wizard1-tab" href="#wizard1" data-toggle="tab" role="tab" aria-controls="wizard1" aria-selected="true">
                            ...
                            <div class="wizard-step-text">
                                ...
                                <div class="wizard-step-text-details text-dark">Lengkapi Formulir Perkawinan dalam Desa Adat Berikut Ini</div>
                            </div>
                        </a>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-xxl-8 col-xl-8 mt-4">
                            <form id="form-create-perkawinan" method="post" action="{{route('desa-perkawinan-dalam-desa-adat-store')}}" enctype="multipart/form-data" class="needs-validation" novalidate>
                                @csrf 
                                <div id="perkawinan_dalam_desa_adat">
                                    ...
                                    <div class="form-group">
                                        <label for="banjar_adat_pradana">Banjar Adat Pradana<span class="text-danger small">*</span></label>
                                        <select class="select2 custom-select @error ('banjar_adat_pradana') is-invalid @enderror" name="banjar_adat_pradana" id="banjar_adat_pradana"  style="width: 100%" required>
                                            <option value="">Pilih Banjar Adat</option>
                                            @foreach($banjar_adat as $banjar)
                                            <option value="{{ $banjar->id }}" @if(old('banjar_adat_pradana') == $banjar->id) selected @endif>{{ $banjar->nama_banjar_adat }}</option>
                                            @endforeach
                                        </select>
                                        ...
                                        @else
                                            <div class="invalid-feedback">
                                                Banjar Adat Pradana wajib dipilih
                                            </div>
                                        @enderror 
                                    </div>
                                ...
                                <div class="d-flex justify-content-between mb-2">
                                    ...
                                    <a class="btn btn-danger mr-2" href="{{ route('desa-perkawinan-dalam-desa-adat-home') }}">Kembali</a><button class="btn btn-success" type="submit">Simpan</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
@push('js')
    ...
    <script>
        $(document).ready( function () {
            ...
            $('#nav-link-perkawinan-dalam-desa').addClass('active');
            ...
            $('#kabupaten_masuk_desa_adat').on('change', function(){
                if($(this).val() != ""){
                    jQuery.ajax({
                        url: "/admin/master/kecamatan/"+$(this).val(),
                        method: 'get',
                        success: function(result){
                            $('#kecamatan_masuk_desa_adat').empty();
                            $('#kecamatan_masuk_desa_adat').append('<option value="">Pilih Kecamatan</option>');
                            result['0'].forEach(element => {
                                $('#kecamatan_masuk_desa_adat').append('<option value="' + element['id'] + '"' +'>' + element['name'] + '</option>');
                            });
                            $('#desa_adat_masuk_desa_adat').empty();
                            $('#banjar_adat_pradana_masuk_desa_adat').empty(); 
                            $('#desa_adat_masuk_desa_adat').append('<option value="">Pilih Desa Adat</option>');
                            $('#banjar_adat_pradana_masuk_desa_adat').append('<option value="" selected>Pilih Banjar Adat</option>');
            ...
            $('#kecamatan_masuk_desa_adat').on('change', function(){
                if($(this).val() != ""){
                    jQuery.ajax({
                        ...
                        success: function(result){
                            ...
                            result.desa_adats.forEach(element => {
                                $('#desa_adat_masuk_desa_adat').append('<option value="' + element['id'] + '"' +'>' + element['desadat_nama'] + '</option>');
                            });  
                            $('#banjar_adat_pradana_masuk_desa_adat').empty(); 
                            $('#banjar_adat_pradana_masuk_desa_adat').append('<option value="" selected>Pilih Banjar Adat</option>');
                            $('#pradana_masuk_desa_adat').prop("disabled", true);                                      
            ...
            $('#desa_adat_masuk_desa_adat').on('change', function(){
                if($(this).val() != ""){
                    ...
                    jQuery.ajax({
                        ...
                        success: function(result){
                            ...
                            result.banjar_adats.forEach(element => {
                                $('#banjar_adat_pradana_masuk_desa_adat').append('<option value="' + element['id'] + '"' +'>' + element['nama_banjar_adat'] + '</option>');
                            });                     
                        }
                    });
                }
            ...
            $('#banjar_adat_purusa_masuk_desa_adat').on('change', function(){
                if($(this).val() == ''){
                    $('#purusa_masuk_desa_adat').prop("disabled", true);
                }else if($(this).val() != ''){
                    $('#purusa_masuk_desa_adat').prop("disabled", false);
                }
            });

            $('#banjar_adat_pradana_masuk_desa_adat').on('change', function(){
                if($(this).val() == ''){
                    $('#pradana_masuk_desa_adat').prop("disabled", true);
                }else if($(this).val() != ''){
            ...
            //PERKAWINAN KELUAR DESA ADAT LISTENER
            $('#kabupaten_keluar_desa_adat').on('change', function(){
                if($(this).val() != ""){
                    jQuery.ajax({
                        url: "/admin/master/kecamatan/"+$(this).val(),
                        method: 'get',
                        success: function(result){
                            $('#kecamatan_keluar_desa_adat').empty();
                            $('#kecamatan_keluar_desa_adat').append('<option value="">Pilih Kecamatan</option>');
                            result['0'].forEach(element => {
                                $('#kecamatan_keluar_desa_adat').append('<option value="' + element['id'] + '"' +'>' + element['name'] + '</option>');
                            });
                            $('#desa_adat_keluar_desa_adat').empty();
                            $('#banjar_adat_pradana_keluar_desa_adat').empty(); 
                            $('#desa_adat_keluar_desa_adat').append('<option value="">Pilih Desa Adat</option>');
                            $('#banjar_adat_pradana_keluar_desa_adat').append('<option value="" selected>Pilih Banjar Adat</option>');
                            $('#purusa_keluar_desa_adat').prop("disabled", true);                   
                        }
                    });
                }
            });

            $('#kecamatan_keluar_desa_adat').on('change', function(){
                if($(this).val() != ""){
                    jQuery.ajax({
                        url: "/admin/master/desa-adat/"+$(this).val(),
                        method: 'get',
                        success: function(result){
                            ...
                            $('#desa_adat_keluar_desa_adat').append('<option value="">Pilih Desa Adat</option>');
                            result.desa_adats.forEach(element => {
                                $('#desa_adat_keluar_desa_adat').append('<option value="' + element['id'] + '"' +'>' + element['desadat_nama'] + '</option>');
                            });  
                            $('#banjar_adat_pradana_keluar_desa_adat').empty(); 
                            $('#banjar_adat_pradana_keluar_desa_adat').append('<option value="" selected>Pilih Banjar Adat</option>');
                            $('#purusa_keluar_desa_adat').prop("disabled", true);                                      
            ...
            $('#desa_adat_keluar_desa_adat').on('change', function(){
                if($(this).val() != ""){
                    var url = "{{ route('desa-banjar-adat-get', ":id") }}";
                    url = url.replace(':id', $(this).val());
                    jQuery.ajax({
                        ...
                        success: function(result){
                            console.log(result);
                            $('#banjar_adat_purusa_keluar_desa_adat').empty();
                            $('#banjar_adat_purusa_keluar_desa_adat').append('<option value="" selected>Pilih Banjar Adat</option>');
                            result.banjar_adats.forEach(element => {
                                $('#banjar_adat_purusa_keluar_desa_adat').append('<option value="' + element['id'] + '"' +'>' + element['nama_banjar_adat'] + '</option>');
                            });    
                            $('#purusa_keluar_desa_adat').prop("disabled", true);                    
                        }
                    });
            ...
            $('#banjar_adat_purusa_keluar_desa_adat').on('change', function(){
                if($(this).val() == ''){
                    $('#purusa_keluar_desa_adat').prop("disabled", true);
                }else if($(this).val() != ''){
                    $('#purusa_keluar_desa_adat').prop("disabled", false);
                }
            });

            $('#banjar_adat_pradana_keluar_desa_adat').on('change', function(){
                if($(this).val() == ''){
                    $('#pradana_keluar_desa_adat').prop("disabled", true);
                }else if($(this).val() != ''){
                    $('#pradana_keluar_desa_adat').prop("disabled", false);
                }
            });
            //AKHIR PERKAWINAN KELUAR DESA ADAT LISTENER
