            ...
            @foreach($krama_mipil as $krama)
                <tr>
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-success btn-sm"  data-toggle="tooltip" data-placement="top" title="" data-original-title="Pilih" onclick="tambah_prajuru_desa_adat_pilih_krama({{ $krama->id }}, '{{ $krama->cacah_krama_mipil->penduduk->gelar_depan }} {{ $krama->cacah_krama_mipil->penduduk->nama }}@if($krama->cacah_krama_mipil->penduduk->gelar_belakang != ''), {{ $krama->cacah_krama_mipil->penduduk->gelar_belakang }}@endif')"><i class="fas fa-user-check mr-1"></i>Pilih</button>
                    </td>
                </tr>
            @endforeach
        </tbody>
