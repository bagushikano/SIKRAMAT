            ...
            @foreach($kramas as $krama)
                <tr>
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-success btn-sm"  data-toggle="tooltip" data-placement="top" title="" data-original-title="Pilih" onclick="pilih_cacah_krama({{ $krama->id }}, '{{ $krama->penduduk->gelar_depan }} {{ $krama->penduduk->nama }}@if($krama->penduduk->gelar_belakang != ''), {{ $krama->penduduk->gelar_belakang }}@endif')"><i class="fas fa-user-check mr-1"></i>Pilih</button>
                </tr>
            @endforeach
        </tbody>
    </table>
