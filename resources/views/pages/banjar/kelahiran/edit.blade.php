    ...
    <div class="container mt-n10">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form id="form-edit-kelahiran" method="post" action="" enctype="multipart/form-data" class="needs-validation" novalidate>
                    @csrf 
                                ...
                                <div class="col-xxl-10 col-xl-10">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6 col-sm-12 py-2">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control @error ('nik') is-invalid @enderror"  id="nik" name="nik" placeholder="Masukkan NIK" value="{{ old('nik', $penduduk->nik) }}">
                                                @error('nik')
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6 col-sm-12 py-2">
                                            ...
                                            @if($kelahiran->file_akta_kelahiran != NULL)   
                                                <a class="text-start text-primary small" href="{{$kelahiran->file_akta_kelahiran}}" target="_blank"><p><i class="fas fa-download"></i> Unduh File Akta Kelahiran</p></a>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <div class="col-xxl-8 col-xl-10">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control @error ('krama_mipil_placeholder') is-invalid @enderror form-custom"  id="krama_mipil_placeholder" name="krama_mipil_placeholder" placeholder="Pilih Krama Mipil" value="{{ old('krama_mipil_placeholder', $krama_mipil->cacah_krama_mipil->penduduk->nama) }}" required readonly>
                                    ...
                                    <div class="form-group" id="ayah_kandung_div">
                                        ...
                                        <select class="select2 custom-select @error ('ayah_kandung') is-invalid @enderror" name="ayah_kandung" id="ayah_kandung"  style="width: 100%" required>
                                            ...
                                            @foreach($anggota_krama_mipil as $anggota)
                                                @if(($anggota->cacah_krama_mipil_id != $cacah_krama_mipil->id) && ($anggota->cacah_krama_mipil->penduduk->jenis_kelamin == 'laki-laki'))
                                                ...
                                                @endif
                                            @endforeach
                                            @foreach($anggota_krama_mipil_meninggal as $anggota)
                                                @if(($anggota->cacah_krama_mipil_id != $cacah_krama_mipil->id) && ($anggota->cacah_krama_mipil->penduduk->jenis_kelamin == 'laki-laki'))
                                                    <option value="{{ $anggota->cacah_krama_mipil->penduduk->id }}" @if($anggota->cacah_krama_mipil->penduduk->id == $penduduk->ayah_kandung_id) selected @endif>{{ $anggota->cacah_krama_mipil->penduduk->nama }}</option>
                                                @endif
                                            @endforeach
                                        ...
                                        <div class="custom-control custom-checkbox mb-n2">
                                            <input class="custom-control-input" id="tampilkan_semua_ayah" checked type="checkbox">
                                    ...
                                    <div class="form-group" id="ibu_kandung_div">
                                        ...
                                        <select class="select2 custom-select @error ('ibu_kandung') is-invalid @enderror" name="ibu_kandung" id="ibu_kandung"  style="width: 100%" required>
                                            ...
                                            @endif                                            
                                            @foreach($anggota_krama_mipil as $anggota)
                                                @if(($anggota->cacah_krama_mipil_id != $cacah_krama_mipil->id) && ($anggota->cacah_krama_mipil->penduduk->jenis_kelamin == 'perempuan'))
                                        ...
                                        <div class="custom-control custom-checkbox mb-n2">
                                            <input class="custom-control-input" id="tampilkan_semua_ibu" checked type="checkbox">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <div>
                                            <button class="btn btn-success btn-icon-split text-end" onclick="simpan_kelahiran({{ $kelahiran->id }}, '0')">
                                                <span class="icon">
                                            ...
                                            <button class="btn btn-success btn-icon-split text-end" onclick="simpan_kelahiran({{ $kelahiran->id }}, '1')">
                                                <span class="icon">
                                                    <i class="fas fa-save"></i>
                                                </span>
...
<script type='text/javascript' src="http://ajax.aspnetcdn.com/ajax/jquery.validate/1.8/jquery.validate.min.js"></script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#tampilkan_semua_ayah").click(function(){
                if($("#tampilkan_semua_ayah").is(':checked') ){
                    ...
                    if('{{ $krama_mipil->cacah_krama_mipil->penduduk->jenis_kelamin }}' == 'laki-laki'){
                        $('#ayah_kandung').append('<option value="{{ $krama_mipil->cacah_krama_mipil->penduduk->id }}"@if($krama_mipil->cacah_krama_mipil->penduduk->id == $cacah_krama_mipil->penduduk->ayah_kandung_id) selected @endif>{{ $krama_mipil->cacah_krama_mipil->penduduk->nama }}</option>');
                    }
                    var ayah = {!! json_encode($anggota_krama_mipil->toArray()) !!}
                    ayah.forEach(element => {
                    ...
                    var ayah = {!! json_encode($anggota_krama_mipil_meninggal->toArray()) !!}
                    ayah.forEach(element => {
                        if(element.cacah_krama_mipil.penduduk.jenis_kelamin == 'laki-laki'){
                            $('#ayah_kandung').append('<option value="'+element.cacah_krama_mipil.penduduk.id+'">'+element.cacah_krama_mipil.penduduk.nama+'</option>');
                        }
            ...
            $("#tampilkan_semua_ibu").click(function(){
                if($("#tampilkan_semua_ibu").is(':checked') ){
                    ...
                    if('{{ $krama_mipil->cacah_krama_mipil->penduduk->jenis_kelamin }}' == 'perempuan'){
                        $('#ibu_kandung').append('<option value="{{ $krama_mipil->cacah_krama_mipil->penduduk->id }}"@if($krama_mipil->cacah_krama_mipil->penduduk->id == $cacah_krama_mipil->penduduk->ibu_kandung_id) selected @endif>{{ $krama_mipil->cacah_krama_mipil->penduduk->nama }}</option>');
                    }
                    
                    var ibu = {!! json_encode($anggota_krama_mipil->toArray()) !!}
                    ibu.forEach(element => {
                    ...
                    var ibu = {!! json_encode($anggota_krama_mipil_meninggal->toArray()) !!}
                    ibu.forEach(element => {
                        if(element.cacah_krama_mipil.penduduk.jenis_kelamin == 'perempuan'){
                            $('#ibu_kandung').append('<option value="'+element.cacah_krama_mipil.penduduk.id+'">'+element.cacah_krama_mipil.penduduk.nama+'</option>');
                        }
        ...
        function simpan_kelahiran(id, status){
            var url = "{{ route('banjar-kelahiran-update', [":id", ":status"]) }}";
            url = url.replace(':id', id);
            url = url.replace(':status', status);
            $("#form-edit-kelahiran").attr("action", url);
            $('#form-edit-kelahiran').submit(function (e){
                e.stopPropagation();
            });
        }
